...
function applyLang(lang: Lang): void {
  ...
  document.querySelectorAll<HTMLOptionElement>('[data-label-fr]').forEach((opt) => {
    const fr = opt.dataset.labelFr;
    const ar = opt.dataset.labelAr;
    if (fr && ar) opt.textContent = lang === 'ar' ? ar : fr;
  });
